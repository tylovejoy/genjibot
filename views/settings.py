...
class SettingsView(discord.ui.View):
    ...
    def __init__(
        self, original_itx: discord.Interaction[core.Genji], flags: int
    ) -> None:
        super().__init__(timeout=3600)
        self.itx = original_itx
        self.flags = cache.SettingFlags(flags)
        self.verification = NotificationButton(
            "Verification", cache.SettingFlags.VERIFICATION in self.flags
        )
        self.add_item(self.verification)
        self.promotion = NotificationButton(
            "Promotion", cache.SettingFlags.PROMOTION in self.flags
        )
...
class NotificationButton(discord.ui.Button):
    ...
    async def callback(self, itx: discord.Interaction[core.Genji]) -> None:
        ...
        self.edit_button(
            self.name, getattr(cache.SettingFlags, self.name.upper()) in self.view.flags
        )
