# This file has been adapted from RoboDanny created by Rapptz
...
import datetime
import io
from typing import TYPE_CHECKING, Annotated, Any, Iterable, Optional, TypeAlias, TypedDict
...
async def check_guild_permissions(ctx: GenjiCtx, perms: dict[str, bool], *, check=all):
...
class TagPages(SimplePages):
    def __init__(self, entries: list[TagEntry], *, ctx: GenjiCtx, per_page: int = 12):
...
class TagMakeModal(discord.ui.Modal, title="Create New Tag"):
    ...
    def __init__(self, cog: Tags, ctx: GenjiCtx):
        super().__init__()
        self.cog: Tags = cog
...
class Tags(commands.Cog):
    ...
    async def cog_command_error(self, ctx: GenjiCtx, error: commands.CommandError):
    ...
    async def get_tag(
        ...
        *,
        ...
        pool = pool or self.bot.database.pool
    ...
    async def tag(self, ctx: GenjiCtx, *, name: Annotated[str, TagName(lower=True)]):
    ...
    @commands.guild_only()
    async def make(self, ctx: GenjiCtx):
    ...
    @make.error
    async def tag_make_error(self, ctx: GenjiCtx, error: commands.CommandError):
        ...
        *,
        ...
        if content is None:
            if ctx.interaction is None:
                ...
                row: Optional[tuple[str]] = await self.bot.database.pool.fetchrow(
    ...
    async def remove(self, ctx: GenjiCtx, *, name: Annotated[str, TagName(lower=True)]):
        ...
        status = await self.bot.database.pool.execute(query, *args)
    ...
    async def remove_id(self, ctx: GenjiCtx, tag_id: int):
        """Removes a tag by ID.
    ...
    async def _send_alias_info(self, ctx: GenjiCtx, record: asyncpg.Record):
    ...
    async def _send_tag_info(self, ctx: GenjiCtx, record: asyncpg.Record):
        embed = discord.Embed(colour=discord.Colour.blurple())
        ...
        rank = await self.bot.database.pool.fetchrow(query, record["id"])
    ...
    @app_commands.autocomplete(name=aliased_tag_autocomplete)
    async def info(self, ctx: GenjiCtx, *, name: Annotated[str, TagName(lower=True)]):
        ...
        record = await self.bot.database.pool.fetchrow(query, name, ctx.guild.id)
        if record is None:
    ...
    async def raw(self, ctx: GenjiCtx, *, name: Annotated[str, TagName(lower=True)]):
    ...
    async def _list(self, ctx: GenjiCtx, *, member: discord.User = commands.Author):
        ...
        rows = await self.bot.database.pool.fetch(query, ctx.guild.id, member.id)
    ...
    async def _tag_all_text_mode(self, ctx: GenjiCtx):
    ...
    @commands.guild_only()
    async def _all(self, ctx: GenjiCtx, *, flags: TagAllFlags):
        ...
        rows = await self.bot.database.pool.fetch(query, ctx.guild.id)
    ...
    async def purge(self, ctx: GenjiCtx, member: discord.User):
        ...
        row: tuple[int] = await self.bot.database.pool.fetchrow(query, ctx.guild.id, member.id)  # type: ignore
    ...
    async def search(self, ctx: GenjiCtx, *, query: Annotated[str, commands.clean_content]):
        ...
        results = await self.bot.database.pool.fetch(sql, ctx.guild.id, query)
    ...
    async def claim(self, ctx: GenjiCtx, *, tag: Annotated[str, TagName]):
        ...
        row = await self.bot.database.pool.fetchrow(query, ctx.guild.id, tag.lower())
        if row is None:
            alias_query = "SELECT tag_id, owner_id FROM tag_lookup WHERE location_id = $1 and LOWER(name) = $2;"
            row = await self.bot.database.pool.fetchrow(alias_query, ctx.guild.id, tag.lower())
            if row is None:
