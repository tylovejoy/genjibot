...
COMPLETION_SUBMISSIONS_INFO = discord.Embed(
    ...
    description=(
        ...
        "\n\n"
        "- By using the `time` argument, you can track your personal bests and "
        "compare them to others using the `/completions` command.\n"
        "- Additionally, you must rate the quality of the map. Use the `quality` "
        "argument to rate the map on a scale from 1 to 6:\n"
        " - 6: Excellent\n"
        " - 5: Great\n"
        " - 4: Good\n"
        " - 3: Average\n"
...
MEDALS_INFO = discord.Embed(
    title="Medals Info",
    description=(
        "- To get a +, ++ or +++ rank, you must obtain the same amount of **Bronze**, **Silver**, or **Gold** "
        "medals as the rank normally requires (see image below)."
...
COMPLETION_SUBMISSION_RULES = discord.Embed(
    ...
    description=(
        ...
        "- Time must be displayed in either the Top 5 leaderboard, or as the announcement in the middle of the screen. "
        "If video submission, it must show both.\n"
        "- You cannot use edit the map in anyway using Custom Games settings, Workshop Settings, "
        "or any other Workshop code. This includes but is not limited to changing tech bans, gravity, etc.\n"
        "- You are not allowed to use scripts, macros, or anything similar to complete any portion of a map.\n"
        "- You may not used a banned tech (restricted via map author/listed in @GenjiBot#9209) where the "
        "ban is non-functional due to Workshop bugs.\n\n"
        "*Records Only:*\n"
        ...
        "- Editing before and after is allowed but it ***cannot*** interfere with timer or any ability "
